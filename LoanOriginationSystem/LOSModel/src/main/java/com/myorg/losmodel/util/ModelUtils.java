...
public class ModelUtils {
	...
	public static void initializeDBtoObjectModelMapping(MortgageApplication ma) {
		...
		dbAttributeToObjectNamesMap = new HashMap<String, String>();

		readAllFields(Arrays.asList(ma.getClass().getDeclaredFields()), ma, null, null, dbAttributeToObjectNamesMap,
		...
		try {
			...
			for (Field field : fields) {
				...
				if (field.getType().isPrimitive() || field.getType().getName().equals("java.lang.String")) {
					...
					//System.out.println("Field Name ->" + field.getName() + " Primitive Value ->" + field.get(obj));
				...
				while (st1.hasMoreTokens()) {
					...
					if (attrName.equals(field.getName())) {
						...
						if (dbFieldToObjectModelMappings != null) {
							...
							//System.out.println(parentClassName + "@" + field.getName());
						}
					}
				}
			}
